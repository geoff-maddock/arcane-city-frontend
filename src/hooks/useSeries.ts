...
export const useSeries = ({ page = 1, itemsPerPage = 25, filters, sort = 'name', direction = 'asc' }: UseSeriesParams = {}) => {
    return useQuery<PaginatedResponse<Series>>({
        ...
        queryFn: async () => {
            ...
            if (filters?.venue) params.append('filters[venue]', toKebabCase(filters.venue));
            if (filters?.promoter) params.append('filters[promoter]', toKebabCase(filters.promoter));
            ...
            if (filters?.occurrence_type) params.append('filters[occurrence_type]', toKebabCase(filters.occurrence_type));
            if (filters?.occurrence_week) params.append('filters[occurrence_week]', toKebabCase(filters.occurrence_week));
            if (filters?.occurrence_day) params.append('filters[occurrence_day]', toKebabCase(filters.occurrence_day));
